...
public class AppApiService extends CDZBaseController {
	...
	public void demo(HttpModel httpModel) {
		Dto qDto = httpModel.getInDto();  //请求入参Dto
		Dto odto = Dtos.newDto();  //创建一个常规的Dto对象
		String is_del=qDto.getString("is_del");   //得到手机号的键值对，APP端发送过来的手机号对应的参数就是is_del
		Dto pDto=Dtos.newDto("is_del", is_del);   //创建一个常规的Dto对象，并初始化一个键值对
		...
		httpModel.setOutMsg(AOSJson.toJson(odto));   //把odto发送回APP端
	}
	...
	public void userLogin(HttpModel httpModel) {
		...
		if(AOSUtils.isEmpty(phone)){
			...
			//dto.put(AOSCons.APPCODE_KEY, AOSCons.SUCCESS);
				dto.put("token", "150e18b7fdf84cb18cb2493b0d354f16");
				dto.put("uid", "f84cb18cb2493b0d354f12");
				dto.put("msg", "成功");
				dto.put("status", "1");
	 ...
	 * 会员登录
	 */
	public void loginUser(HttpModel httpModel) {
		Dto qDto = httpModel.getInDto();
		Dto odto = Dtos.newDto();
		...
		if(!qDto.containsKey("account")||AOSUtils.isEmpty(account)){
			...
			if(null==aosUserPO){
			...
			}else if(!StringUtils.equals(password_, aosUserPO.getPassword_())){
				...
				odto.put(AOSCons.APPCODE_KEY, AOSCons.SUCCESS);
				odto.put("juid", juid);
				odto.put("uid", aosUserPO_.getId_());
				odto.put(AOSCons.APPMSG_KEY, AOSCons.APPMSG_SUCCESS);
			}
		}
		httpModel.setOutMsg(AOSJson.toJson(odto));
